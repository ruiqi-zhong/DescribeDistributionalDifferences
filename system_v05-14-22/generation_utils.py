...
class GenerationMixin:
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        is_greedy_gen_mode = (num_beams == 1) and (num_beam_groups == 1) and do_sample is False and ensemble_sample is False
        is_sample_gen_mode = (num_beams == 1) and (num_beam_groups == 1) and do_sample is True and ensemble_sample is False
        is_beam_gen_mode = (num_beams > 1) and (num_beam_groups == 1) and do_sample is False and ensemble_sample is False
        is_beam_sample_gen_mode = (num_beams > 1) and (num_beam_groups == 1) and do_sample is True and ensemble_sample is False
        is_group_beam_gen_mode = (num_beams > 1) and (num_beam_groups > 1) and ensemble_sample is False
        is_ensemble_sample_mode = ensemble_sample
        ...
        if is_greedy_gen_mode:
            ...
            # index [prompt1sample1, prompt1sample2, prompt1sample3 ....., ]
            # .reshape(num_prompts, num_samples) will return the correct result
            # sample
            return self.ensemble_sample(
                input_ids,
    ...
    def ensemble_sample(
        self,
        input_ids: torch.LongTensor,
        num_samples: int, # we require this argument; number of prompts to ensemble
        num_return_sequences: int, # we require this argument; number of return sequences
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        logits_warper: Optional[LogitsProcessorList] = None,
        max_length: Optional[int] = None,
        ...
        output_scores: Optional[bool] = None,
        return_dict_in_generate: Optional[bool] = None,
        ensemble_method: str = 'prob', # or we can ensemble the logits
        **model_kwargs,
    ) -> Union[SampleOutput, torch.LongTensor]:
        assert len(input_ids) == num_samples * num_return_sequences
        assert ensemble_method in ('prob', 'logit')
        r"""
        Generates sequences for models with a language modeling head using multinomial sampling.
        ...
        while cur_len < max_length:
            ...
            # num_prompts X num_return_sequences
            next_token_logits = outputs.logits[:, -1, :]
            ...
            if ensemble_method == 'logit':
                next_token_scores = torch.mean(next_token_scores.reshape((num_samples, num_return_sequences, -1)), axis=0)

            # sample
            probs = F.softmax(next_token_scores, dim=-1)
            if ensemble_method == 'prob':
                probs = probs.reshape((num_samples, num_return_sequences, -1))
                probs = torch.mean(probs, axis=0)

            next_tokens = torch.multinomial(probs, num_samples=1).squeeze(1)
            next_tokens = next_tokens.repeat(num_samples)
