...
def train(cv_dict):
    ...
    for step in tqdm.trange(NUM_STEPS):
        ...
        if step % 10 == 0:
            fpr, tpr, thresholds = roc_curve(labels.detach().cpu().numpy(), logits.detach().cpu().numpy()[:,1])
...
def evaluate(texts, use_shap: bool, model, tokenizer):
    ...
    if not use_shap:
        ...
        def predict(x):
            tv = torch.tensor(
                [tokenizer(v, padding=True, max_length=max_length, truncation=True) for v in x]
            ).to(device)
