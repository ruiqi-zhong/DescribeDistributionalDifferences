...
def get_rep(
    ...
    if save_folder is None:
        save_folder = os.path.join("end2end_jobs_benchmark_1018_new", pair)
    ...
    extreme_vals = return_extreme_values(pos, neg, use_shap=False, top_p=top_p)
    # pkl.dump(
...
if __name__ == "__main__":
    ...
    type = pair["type"]
    print(f"Running get_rep on category type {type} at top percent {top_p}")
    get_rep(
        pos=pair["positive_samples"],
        neg=pair["negative_samples"],
        pair="benchmark_1018-" + str(top_p) + "-" + type + "-" + str(index),
        top_p=top_p,
    )
